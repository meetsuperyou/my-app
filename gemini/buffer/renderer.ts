// src/renderer.ts
import {TerminalState} from "./models/terminal.model";
 ...
 */
export function renderTerminal(terminalJson: string, containerElement: HTMLElement): void
{
  const state: TerminalState = JSON.parse(terminalJson);
  // 清空現有內容
  ...
  state.grid.forEach((row, y) =>
  {
    ...
    row.forEach((cell, x) =>
    {
      ...
      if (bg) cellDiv.style.backgroundColor = bg;
      if (cell.attrs.bold) cellDiv.style.fontWeight = "bold";
      if (cell.attrs.italic) cellDiv.style.fontStyle = "italic";
      if (cell.attrs.underline) cellDiv.style.textDecoration = "underline";
